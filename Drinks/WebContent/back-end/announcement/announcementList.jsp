...
<title>公告列表</title>
<link rel="stylesheet"
	href="<%=request.getContextPath()%>/back-end/css/bootstrap.css">
</head>
<body>
	<div class="container">
		<div class="row">
	 	    ...
	 	    <table class="table table-bordered table-hover">
				...
				<c:forEach var="announcementVO" items="${list}">
					<tr>
						...
						<td><c:if test="${empty announcementVO.ann_img}">
								<img style="width: 100px; height: 100px;"
									src="<%=request.getContextPath()%>/img/test.jpg">
							</c:if> <c:if test="${! empty announcementVO.ann_img}">
								<img
									src="data:image/png;base64,${announcementVO.getImg(announcementVO.ann_img)}"
									width="100" height="100" />
							</c:if></td>
						<td>
							<FORM METHOD="post"
								ACTION="<%=request.getContextPath()%>/back-end/announcement.do">
								<input type="submit" value="修改" class="btn btn-primary">
								<input type="hidden" name="ann_id"
									value="${announcementVO.ann_id}"> <input type="hidden"

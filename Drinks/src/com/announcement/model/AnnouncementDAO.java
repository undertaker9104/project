...
public class AnnouncementDAO implements AnnouncementDAO_interface{
	...
	private static final String INSERT_STMT = "INSERT INTO ANNOUNCEMENT (ann_id,ANN_TITLE,ann_des,ANN_DATE,ann_img) VALUES ('AI'||LPAD(to_char(announcement_seq.NEXTVAL), 6, '0'), ?, ?, ?, ?)";
	...
	private static final String UPDATE = "UPDATE ANNOUNCEMENT set ANN_TITLE=?, ann_des=?, ANN_DATE=?, ann_img=? where ann_id = ?";
	private static final String SELECT_PIC = "SELECT ANN_IMG FROM ANNOUNCEMENT where ANN_ID = ? ";
		...
		public void update(AnnouncementVO announcementVO) {
			...
			try {
				...
				pstmt.setDate(3, announcementVO.getAnn_date());
				pstmt.setBytes(4, announcementVO.getAnn_img());
				pstmt.setString(5, announcementVO.getAnn_id());
